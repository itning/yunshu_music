...
class MediaPlayerImpl : NSObject{
        ...
        if keyPath == "status" {
            ...
            case .readyToPlay:
                ...
                if let music = MusicPlayDataService.INSTANCE.getNowPlayMusic(){
                    ...
                    setLockScreenDisplay(music:music)
                    if playNow{
                        self.play()
                    }else{
    ...
    private func setLockScreenDisplay(music:Music) -> Void {
        DispatchQueue.global().async {
            
            var data:Data?
            do{
                if let url = URL(string: music.coverUri){
            ...
            DispatchQueue.main.async {
                if data != nil {
                    ...
                    let metadata: [String: Any] = [
                        ...
                        MPMediaItemPropertyAlbumTitle: music.singer,
                        MPMediaItemPropertyPlaybackDuration: self.playerItem?.duration.seconds ?? 0,
    ...
    private func setLockScreenDisplayTime() -> Void {
        // 获取当前音频的总时长，并设置播放时长信息
        let duration = playerItem?.duration.seconds ?? 0
        // 获取当前音频已经播放的时间，并设置已播放时长信息
        let elapsedTime = playerItem?.currentTime().seconds ?? 0
